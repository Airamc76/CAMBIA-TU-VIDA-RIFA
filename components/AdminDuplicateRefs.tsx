...
const AdminDuplicateRefs: React.FC<{ onSearchRef: (ref: string) => void }> = ({ onSearchRef }) => {
    ...
    if (duplicates.length === 0 && !isLoading) return null;
            ...
            <div className="bg-rose-950/20 border-2 border-rose-500/30 rounded-[3rem] p-8 md:p-10 relative overflow-hidden backdrop-blur-sm">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 relative z-10">
                    ...
                    <Button
                        ...
                        disabled={isLoading}
                        variant="blue"
                        className="rounded-2xl px-6 py-3 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 bg-rose-500 hover:bg-rose-400"
                    >
                        ...
                        <div key={dup.reference} className="bg-slate-900/60 border border-rose-500/20 p-6 rounded-[2rem] hover:border-rose-500/50 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-2xl font-black text-white tracking-widest group-hover:text-rose-400 transition-colors">{dup.reference}</p>
                                </div>
