-- 🎟️ ROBUST FIX: RAFFLE CREATION & SYNC 🎟️
-- Adds NULLIF and safer type casting to prevent 400 Bad Request errors.

-- 1. Unify Trigger Function (Unchanged but ensuring it's here)
...
-- 2. Update save_raffle RPC (Safer version)
...
BEGIN
  ...
  -- 🛠️ Safer Type Parsing
  -- Use NULLIF to handle empty strings from frontend
  ...
  IF EXISTS (SELECT 1 FROM public.raffle_numbers WHERE raffle_id = v_raffle_id) AND
     NOT EXISTS (SELECT 1 FROM public.raffle_numbers WHERE raffle_id = v_raffle_id AND status = 'available') THEN
